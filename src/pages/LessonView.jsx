...
// مخزن الأنشطة البديلة التوليدية من كودك الأصلي
...
const distinctionActivitiesPool = [
  "لعبة المظلة والمطر المعرفي: يرسم المعلم دائرة عملاقة على السبورة باسم 'مظلة المال الكبرى'، ويكتب خارجها أصولاً مبعثرة (ذهب، شيك، ساعة، قمح، دينار). يطلب من الطلاب إدخال الأصول التي تصلح كمال داخلها، وتلوين العملات الحكومية بلون مستقل ليدركوا أن النقود جزء من مظلة المال الأشمل.",
...
function LessonView({ lessonData, teacherName, onBackToDashboard }) {
  // --- States لإدارة الأدوات التوليدية والتفاعل ---
  const [lessonDuration, setLessonDuration] = useState(45);
  const [activeSmartTab, setActiveSmartTab] = useState('plan');
  const [activeStep, setActiveStep] = useState(0);
  const [timeLeft, setTimeLeft] = useState(2400); // 40 دقيقة بالثواني
  ...
  // حالات التحكم في أزرار توليد الأنشطة الـ 3 المختلفة لكل فقرة
  ...
  // قائمة الـ 40 طالباً لإدارة الصفوف الكثيفة بالقرعة العشوائية
  const students = [
  ...
  // خطوات التحضير والخطة المقترنة بالمؤقت الزمني
  const steps = [
    { id: 1, title: "التهيئة واستدراج المعارف (عصف ذهني)", duration: "5 دقائق", durationSeconds: 300, objective: "إثارة انتباه الطلاب للموضوع وربطه بخبراتهم الحياتية.", activity: "طرح سؤال عام مثير للتفكير: 'لو كنت مسؤولاً عن ميزانية الأسرة، كيف ستتصرف؟'.", tips: "شجع الطلاب على تقديم إجابات متنوعة دون إصدار أحكام فورية." },
    { id: 2, title: "الاستكشاف وبناء المفاهيم المدرسية", duration: "10 دقائق", durationSeconds: 600, objective: "استنتاج القوانين والمبادئ الأساسية للدرس بشكل جماعي صفي.", activity: "قراءة ومناقشة فقرات الدرس الرسمية وتكليف الطلاب بتلخيص النقاط الهامة في دفاترهم المخصصة.", tips: "اكتب المصطلحات الرئيسية على السبورة وتأكد من تدوينها من قِبل الجميع." },
    { id: 3, title: "النشاط التطبيقي للمجموعات الكثيفة", duration: "15 دقيقة", durationSeconds: 900, objective: "تطبيق المبادئ النظرية للدرس في ورقة عمل عملية وصفر تكلفة.", activity: "توزيع أوراق عمل تتضمن سيناريوهات مالية تطبيقية، والطلب من الطلاب العمل في مجموعات الدليل الحركي.", tips: "تجول بين المجموعات لتقديم الدعم والتأكد من مشاركة الجميع." },
    { id: 4, title: "الالتقاط والتقييم التكويني", duration: "7 دقائق", durationSeconds: 420, objective: "التحقق من استيعاب الطلاب للمفاهيم الأساسية وتصحيح الأخطاء الشائعة.", activity: "عرض أسئلة سريعة وصح/خطأ على السبورة وإجابة المجموعات عنها بشكل تنافسي ممتع ومحفز.", tips: "ركز على النقاط التي أظهر الطلاب صعوبة في فهمها أثناء النشاط العملي." },
    { id: 5, title: "الغلق وتذاكر الخروج منظم حركياً", duration: "3 دقائق", durationSeconds: 180, objective: "تلخيص نقاط الدرس وتكليف الطلاب بالتقييم الذاتي والواجب المنزلي المعياري.", activity: "طلب تلخيص شفهي سريع من أحد الطلاب، وتعيين أسئلة 'أقيم تعلمي' الرسمية كواجب مدرسي للبيت.", tips: "أثنِ على الطلاب المتميزين وقدم التغذية الراجعة الإيجابية الشاملة للصف." }
  ];

  // إدارة تفاعلات المؤقت التنازلي التكتيكي للحصة
  useEffect(() => {
...
شارك الطلاب بنشاط حركي غامر ومنظم داخل الغرفة الصفية لتجسيد المفاهيم والتداول والعمل التعاوني بمسؤولية كاملة وصفر تكلفة.
...
*معلم المادة: الأستاذ ${teacherName || ""}* 👨‍🏫`;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px', fontFamily: 'Cairo, sans-serif' }}>
      ...
      {/* الرأس العلوي مع زر العودة للداش بورد الرئيسي */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#ffffff', padding: '20px 24px', borderRadius: '16px', border: '1px solid #bae6fd' }}>
        ...
        <button 
          ...
          style={{ backgroundColor: '#ffffff', color: '#0284c7', border: '2px solid #0284c7', padding: '10px 20px', borderRadius: '10px', cursor: 'pointer', fontWeight: 'bold', fontSize: '15px', fontFamily: 'Cairo' }}
        >
          🔙 العودة للوحة التحكم الرئيسية
        </button>
      </div>

      {/* الهيكل الرئيسي المزدوج */}
      <div style={{ display: 'grid', gridTemplateColumns: '1.1fr 1fr', gap: '24px', alignItems: 'start' }}>
        
        {/* الجانب الأيمن: عرض المادة العلمية كاملة */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          <section style={{ backgroundColor: '#ffffff', padding: '24px', borderRadius: '16px', border: '1px solid #bae6fd', borderRight: '5px solid #0369a1' }}>
            ...
            <ul style={{ margin: '0', paddingRight: '20px', listStyleType: 'disc', lineHeight: '1.8' }}>
              {lessonData?.learning_outcomes?.map((outcome, idx) => (
                ...
                <>
                  <li style={{ fontSize: '16px', color: '#2d3748' }}>تعرّف مفهوم المال.</li>
                  <li style={{ fontSize: '16px', color: '#2d3748' }}>التمييز بين أشكال المال.</li>
                  <li style={{ fontSize: '16px', color: '#2d3748' }}>استنتاج الفرق بين مفهوم المال ومفهوم النقود.</li>
                </>
              )}
            </ul>
          </section>
          ...
          <section style={{ backgroundColor: '#ffffff', padding: '24px', borderRadius: '16px', border: '1px solid #bae6fd' }}>
            <h3 style={{ fontSize: '20px', color: '#0369a1', margin: '0 0 12px 0', fontWeight: 'bold', borderBottom: '1px solid #edf2f7', paddingBottom: '6px' }}>🧭 {lessonData?.exploration_stage?.title || "أستكشف"}</h3>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              {lessonData?.exploration_stage?.prompts?.map((prompt, idx) => (
                ...
                <>
                  <p style={{ fontSize: '16px', margin: 0, lineHeight: '1.7', color: '#2d3748' }}>• هل فكَّرْتُ يومًا كيفَ يُؤثِّرُ المالُ في حياتي؟</p>
                  <p style={{ fontSize: '16px', margin: 0, lineHeight: '1.7', color: '#2d3748' }}>• هل فكَّرْتُ كيفَ يُمكنني استخدام المالِ بحكمة؟</p>
                </>
              )}
            </div>
          </section>
          ...
          <section style={{ backgroundColor: '#ffffff', padding: '24px', borderRadius: '16px', border: '1px solid #bae6fd' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginTop: '10px' }}>
              {lessonData?.core_vocabulary?.map((vocab, idx) => (
                ...
                <>
                  <div style={{ backgroundColor: '#f8fafc', padding: '14px', borderRadius: '10px', borderRight: '4px solid #27ae60' }}>
                    <strong style={{ color: '#27ae60', fontSize: '16px', display: 'block', marginBottom: '4px' }}>المال (Money)</strong>
                    <p style={{ fontSize: '15px', margin: 0, lineHeight: '1.6', color: '#4a5568' }}>يُعرف المالُ بأنَّهُ أي وسيلة تُستخدَمُ لشراء السلع والخدمات، أو سدادِ الديون.</p>
                  </div>
                </>
              )}
            </div>
          ...
          {/* تفاصيل الشرح وأشكال المال وتوليد الأنشطة الثلاثة */}
          <section style={{ backgroundColor: '#ffffff', padding: '24px', borderRadius: '16px', border: '1px solid #bae6fd' }}>
            ...
            <p style={{ fontSize: '16px', lineHeight: '1.7', color: '#2d3748', margin: '0 0 14px 0', textAlign: 'justify' }}>{lessonData?.shapes_of_money?.context || "تعددت أشكال المال وأنواعه على مر العصور في التعاملات المعاصرة."}</p>
            ...
            {lessonData?.shapes_of_money?.types ? (
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '12px', marginBottom: '16px' }}>
                <div style={{ backgroundColor: '#f0f9ff', padding: '12px', borderRadius: '10px', border: '1px solid #e0f2fe' }}>
                  <strong style={{ color: '#0369a1', fontSize: '15px' }}>النقود</strong>
                  <p style={{ fontSize: '13.5px', margin: '4px 0 0 0', color: '#4a5568' }}>أوراق نقديةٌ تُصدِرُها الحكومات القانونية مثل الدينار الأردني.</p>
                </div>
              </div>
            )}
            ...
            <div style={{ borderTop: '1px dashed #bae6fd', paddingTop: '14px', display: 'flex', gap: '8px' }}>
              ...
              {showShapesAct && (
                <button onClick={() => setShapesActIndex((prev) => (prev + 1) % shapesActivitiesPool.length)} style={{ backgroundColor: '#ffffff', color: '#0284c7', border: '1px solid #0284c7', padding: '8px 16px', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold', fontFamily: 'Cairo', fontSize: '13.5px' }}>
                  🔄 توليد خيار بديل آخر (3 أنشطة متاح)
                </button>
              )}
            </div>
            {showShapesAct && (
              <div style={{ backgroundColor: '#eff6ff', padding: '14px', borderRadius: '10px', border: '1px solid #2563eb', marginTop: '12px' }}>
                <strong style={{ display: 'block', fontSize: '14px', color: '#1e40af', marginBottom: '4px' }}>⚙️ نشاط بديل ومبتكر للمعلم (النموذج المقترح رقم {shapesActIndex + 1}):</strong>
                <p style={{ fontSize: '15px', color: '#1e3a8a', margin: 0, lineHeight: '1.7' }}>{shapesActivitiesPool[shapesActIndex]}</p>
              </div>
            )}
          </section>

          {/* الفروق الجوهرية (المال والنقود) وتوليد أنشطتها الثلاثة */}
          <section style={{ backgroundColor: '#ffffff', padding: '24px', borderRadius: '16px', border: '1px solid #bae6fd', borderRight: '5px solid #0284c7' }}>
            <h3 style={{ fontSize: '20px', color: '#0369a1', margin: '0 0 12px 0', fontWeight: 'bold', borderBottom: '1px solid #edf2f7', paddingBottom: '6px' }}>⚖️ {lessonData?.critical_distinction?.title || "الفرق بين المال والنقود"}</h3>
            <p style={{ fontSize: '16px', lineHeight: '1.7', color: '#2d3748', margin: '0 0 12px 0' }}>{lessonData?.critical_distinction?.intro || "يعتقد بعض الأشخاص خطأ أن الكلمتين تحملان المعنى نفسه."}</p>
            ...
            {lessonData?.critical_distinction?.currency_details && (
              <div style={{ backgroundColor: '#f8fafc', padding: '14px', borderRadius: '10px', border: '1px solid #e2e8f0' }}>
                <p style={{ fontSize: '15px', margin: 0, lineHeight: '1.6', color: '#2d3748' }}><strong style={{ color: '#0369a1' }}>النقود:</strong> {lessonData.critical_distinction.currency_details.definition}</p>
              </div>
            )}
            ...
            <div style={{ borderTop: '1px dashed #bae6fd', paddingTop: '14px', display: 'flex', gap: '8px', marginTop: '16px' }}>
              <button onClick={() => { setShowDistinctionAct(!showDistinctionAct); if(!showDistinctionAct) setDistinctionActIndex(0); }} style={{ backgroundColor: '#0284c7', color: '#fff', border: '0', padding: '8px 16px', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold', fontFamily: 'Cairo', fontSize: '13.5px' }}>
                {showDistinctionAct ? "❌ إخفاء النشاط المولد" : "✨ توليد نشاط تمايز بديل لهذه الفقرة"}
              </button>
              {showDistinctionAct && (
            ...
            {showDistinctionAct && (
              <div style={{ backgroundColor: '#eff6ff', padding: '14px', borderRadius: '10px', border: '1px solid #2563eb', marginTop: '12px' }}>
                <strong style={{ display: 'block', fontSize: '14px', color: '#1e40af', marginBottom: '4px' }}>⚙️ نشاط تمايز ودعم الفروق الفردية (النموذج المقترح رقم {distinctionActIndex + 1}):</strong>
        ...
        {/* الجانب الأيسر: إدارة الحصة والمؤقت والتخطيط التنازلي */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            ...
            <div style={{ minHeight: '160px', fontSize: '14px', lineHeight: '1.6' }}>
              {activeSmartTab === 'plan' && (
                <div>
                  ...
                  <div style={{ background: '#f0f9ff', padding: '10px 12px', borderRadius: '6px', borderLeft: '3px solid #0284c7', fontSize: '13.5px', color: '#0369a1' }}>
              ...
              {activeSmartTab === 'kinetic' && (
                <div>
                  ...
                  <p style={{ color: '#475569', margin: '0 0 8px 0', fontSize: '14px' }}>يتم تقسيم الصف الكثيف إلى 4 مجموعات متكافئة لضمان أعلى مشاركة تنظيمية ومنع العشوائية والضوضاء.</p>
                  <span style={{ fontSize: '12.5px', color: '#0284c7', background: '#e0f2fe', padding: '6px 10px', borderRadius: '4px', display: 'inline-block' }}>🚷 <strong>قاعدة الحركة:</strong> يتحرك القادة والمتحدثون الرسميون فقط بالتناوب، ويبقى الـ 36 طالباً في مقاعدهم.</span>
                </div>
              )}
          ...
          <div style={{ backgroundColor: '#ffffff', padding: '24px', borderRadius: '16px', border: '1px solid #bae6fd' }}>
            <strong style={{ fontSize: '14.5px', color: '#1e293b', display: 'block', marginBottom: '6px' }}>🎲 محرك القرعة والسحب العشوائي (تكافؤ الفرص في الصف الكثيف):</strong>
      ...
      {/* 📋 نموذج تحضير الوزارة اليومي الرسمي كامل الحروف دون اقتطاع */}
      <div style={{ textAlign: 'center', padding: '20px 0', borderTop: '1px dashed #bae6fd', marginTop: '10px' }}>
        <button 
          onClick={() => setShowPrepModel(!showPrepModel)} 
          style={{ backgroundColor: '#ffffff', color: '#0284c7', fontWeight: '700', padding: '14px 28px', borderRadius: '12px', fontSize: '16px', border: '2px solid #bae6fd', cursor: 'pointer', fontFamily: 'Cairo' }}
        >
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', direction: 'rtl', border: '1px solid #cbd5e1' }}>
            <tbody>
              ...
              <tr>
                ...
                <td colSpan="2" style={{ padding: '12px', border: '1px solid #cbd5e1' }}>تحديد الفجوات المعرفية وربط خبرات الطلاب بممارساتهم المالية والشخصية اليومية حوارياً.</td>
              </tr>
              <tr>
                ...
                <td colSpan="5" style={{ padding: '12px', border: '1px solid #cbd5e1' }}>
                  <ul style={{ margin: 0, paddingRight: '20px', fontSize: '15px', lineHeight: '1.7', color: '#2d3748' }}>
                    <li style={{ fontSize: '15px', color: '#2d3748' }}>تعرّف مفهوم المال وتمييز ميزاته الواردة في المنهج المعتمد.</li>
                    <li style={{ fontSize: '15px', color: '#2d3748' }}>التمييز الدقيق بين أشكال المال الورقية، الإلكترونية والشيكات.</li>
                    <li style={{ fontSize: '15px', color: '#2d3748' }}>استنتاج الفرق الجوهري والقانوني بين مفهوم المال العام ومفهوم النقود.</li>
                  </ul>
                </td>
              </tr>
            </tbody>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: '12px', border: '1px solid #cbd5e1' }}>
            ...
            <tbody>
              <tr>
                ...
                <td style={{ padding: '12px', border: '1px solid #cbd5e1', fontSize: '14.5px', lineHeight: '1.6' }}>طرح أسئلة عصف ذهني تمهيدية موجهة ومثيرة للتفكير لربط خبرات الطلاب بممارساتهم الحياتية واليومية بالإنفاق.</td>
                <td style={{ padding: '12px', border: '1px solid #cbd5e1', fontSize: '14.5px', lineHeight: '1.6' }}>يستمع بانتباه، ويستحضر ممارساته الحياتية السابقة بالإنفاق والمقصف، ويشارك بنقاش تفاعلي هادئ.</td>
              ...
              <tr>
                ...
                <td style={{ padding: '12px', border: '1px solid #cbd5e1', fontSize: '14.5px', lineHeight: '1.6' }}>تبيان وشرح المفاهيم الحرفية (مثل المال، المقايضة، النقود) وتثبيتها على السبورة ومناقشتها لدعم التمايز بأسلوب علمي رصين وبأمثلة معاصرة.</td>
              ...
              <tr>
                ...
                <td style={{ padding: '12px', border: '1px solid #cbd5e1', fontSize: '14.5px', lineHeight: '1.6' }}>توجيه المجموعات الكثيفة لتنفيذ الأنشطة التعاونية الموضعية التوليدية البديلة المقترحة (مثل محاكاة الحقب التاريخية أو سلسلة الكتل البلوكتشين البشري) وصفر تكلفة.</td>
                <td style={{ padding: '12px', border: '1px solid #cbd5e1', fontSize: '14.5px', lineHeight: '1.6' }}>يعمل تعاونياً بروح الفريق ضمن المجموعات الأربع الموزعة حركياً ويصيغ مع زملائه الحلول والنتائج لسيناريو النشاط.</td>
              ...
              <tr>
                ...
                <td style={{ padding: '12px', border: '1px solid #cbd5e1', fontSize: '14.5px', lineHeight: '1.6' }}>يتأمل في تعلمه الفردي، ويثبت حلول الأسئلة والتدريبات الرسمية على الدفتر، ويسلم تذاكر الخروج للمعلم بشكل منظم حركياً.</td>
                <td style={{ padding: '12px', border: '1px solid #cbd5e1', textAlign: 'center', fontWeight: 'bold' }}>5 دقائق</td>
              </tr>
            </tbody>
          </table>
